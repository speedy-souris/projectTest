...
def management_of_indecency_behavior(chat_object_connect):
    chat_object_connect.calculate_the_indecency_status()
    if chat_object_connect.has_user_indecency_status:
        if chat_object_connect.number_of_user_indecency < 3:
             display_behavior.display_grandpy_status_code_to_disrespectful(
                chat_object_connect)
             counting_behavior.user_indecency_count(chat_object_connect)
        else:
